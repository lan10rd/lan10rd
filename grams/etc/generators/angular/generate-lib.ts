...
let app = async () => {
    try
    {
        ...
        else
        {
            ...
            let demo_path = '@grams/' + demo_lib_name
            ...
            await Promise.all((await files.walk('../../../libs/' + lib_name)).map(async (path:string) => {
                ...
                await files.write(path, file.split(demo_lib_name).join(lib_name).split('@grams').join('@' + nx.npmScope))
            }))

            let module_path = '../../../libs/' + lib_name + '/src/lib/' +  demo_lib_name + '.module.ts'
            await files.erase(module_path)
            files.write(module_path.split(demo_lib_name).join(lib_name), "import { NgModule } from '@angular/core';\nimport { CommonModule } from '@angular/common';\n\n@NgModule({\n\t\timports: [CommonModule],\n})\nexport class " + lib_name + "Module {}")
